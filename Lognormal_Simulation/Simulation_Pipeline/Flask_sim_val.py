...
class Generate_Flask_maps(Kappa_Powerspecrum_T17):
    ...
    def generate_log_normal_maps(self, data):
        ...
        ### loop for cosmology 
        for i in range(200,len(Omega_m)):
    ...
    def generate_log_normal_map(self, Omega_m, sigma_8, cosmo_indx):
        ...
        np.savez("/project/ls-gruen/users/r.kanaki/Masterarbeit/Hirosaki_universe/Flask_Validation_New/Inputs_Flask/Inputs-"+str(cosmo_indx)+"_Flask.npz", **params)
        #### which is input of flask (see prior.config)
        np.savetxt("/project/ls-gruen/users/r.kanaki/Masterarbeit/Hirosaki_universe/Flask_Validation_New/Cl/Cl-"+str(cosmo_indx)+"-f1z1f1z1.dat",np.c_[cl[:,0],cl[:,1]])
    ...
    def use_flask(self, shift_for_sources, cosmo_indx):
        ...
        for i in range(10):
            ...
            seed=cosmo_indx+i*1000
            #### here you choose the directory where you saved .info and .config
            #### here you input the powerspectrum
            ### generate log normal field
            pyFlask.flask(["flask","../Masterarbeit/Juni_2023/New_Flask_Simulation/Flask_val.config","RNDSEED:",str(seed),"DIST:", "LOGNORMAL",
                          "CL_PREFIX:","/project/ls-gruen/users/r.kanaki/Masterarbeit/Hirosaki_universe/Flask_Validation_New/Cl/Cl-"+str(cosmo_indx)+"-"])
            os.chdir("/project/ls-gruen/users/r.kanaki/Masterarbeit/Hirosaki_universe/Flask_Validation_New")
            os.rename("map-f1z1.fits", "Kappa_Field/lognormal/map_realization_"+str(i)+"_cosmos_"+str(cosmo_indx)+"-f1z1.fits")
            os.chdir("/home/r/R.Kanaki/flask")
            ### generate Gaussian field
            pyFlask.flask(["flask","../Masterarbeit/Juni_2023/New_Flask_Simulation/Flask_val.config","RNDSEED:",str(seed),"DIST:", "GAUSSIAN",
                          "CL_PREFIX:","/project/ls-gruen/users/r.kanaki/Masterarbeit/Hirosaki_universe/Flask_Validation_New/Cl/Cl-"+str(cosmo_indx)+"-"])
            os.chdir("/project/ls-gruen/users/r.kanaki/Masterarbeit/Hirosaki_universe/Flask_Validation_New")
            os.rename("map-f1z1.fits", "Kappa_Field/Gauss/map_realization_"+str(i)+"_cosmos_"+str(cosmo_indx)+"-f1z1.fits")
            os.chdir("/home/r/R.Kanaki/flask")