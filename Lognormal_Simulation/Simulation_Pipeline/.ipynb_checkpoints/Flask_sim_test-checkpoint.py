...
class Generate_Flask_maps(Kappa_Powerspecrum_T17):
    ...
    """
    def generate_log_normal_maps(self):
        """
        ...
        Omega_m = self.Omfid
        sigma_8 = self.sig8fid
        ### only one cosmology
        self.generate_log_normal_map(Omega_m, sigma_8)
        executionTime_total = (time.time() - startTime_total)
        print('Execution time in seconds for total: ' + str(executionTime_total))
        
    def generate_log_normal_map(self, Omega_m, sigma_8):
        """
        ...
        np.savez("/project/ls-gruen/users/r.kanaki/Masterarbeit/Hirosaki_universe/Flask_Simulation_Test_Final/Inputs_Flask/Inputs-Flask.npz", **params)
        #### which is input of flask (see prior.config)
        np.savetxt("/project/ls-gruen/users/r.kanaki/Masterarbeit/Hirosaki_universe/Flask_Simulation_Test_Final/Cl-f1z1f1z1.dat",np.c_[cl[:,0],cl[:,1]])
        startTime_flask = time.time()
        self.use_flask(shift_for_sources)
        ...
        os.chdir("/home/r/R.Kanaki/Masterarbeit/מייַ_2023")        
    ...
    def use_flask(self, shift_for_sources):
        """
        ...
        foj=open("/home/r/R.Kanaki/Masterarbeit/מייַ_2023/Flask_simulation/Flask_info.dat","w")
        ...
        for i in range(300):
            #Because the plan is making 10000 cosmos which have different cosmological parameters, and also varying every realization...
            seed=i + 77777 # 777 is lucky number in Japan
            #### here you choose the directory where you saved .info and .config
            #### here you input the powerspectrum
            ### generate log normal field
            pyFlask.flask(["flask","../Masterarbeit/מייַ_2023/Flask_simulation/Flask_validation.config","RNDSEED:",str(seed),"DIST:", "LOGNORMAL",
                          "CL_PREFIX:","/project/ls-gruen/users/r.kanaki/Masterarbeit/Hirosaki_universe/Flask_Simulation_Test_Final/Cl-"])
            os.chdir("/project/ls-gruen/users/r.kanaki/Masterarbeit/Hirosaki_universe/Flask_Simulation")
            os.rename("map-f1z1.fits", "../Flask_Simulation_Test_Final/Kappa_Field/lognormal/map_realization_"+str(i)+"-f1z1.fits")
            os.chdir("/home/r/R.Kanaki/flask")
            ### generate Gaussian field
            pyFlask.flask(["flask","../Masterarbeit/מייַ_2023/Flask_simulation/Flask_validation.config","RNDSEED:",str(seed),"DIST:", "GAUSSIAN",
                          "CL_PREFIX:","/project/ls-gruen/users/r.kanaki/Masterarbeit/Hirosaki_universe/Flask_Simulation_Test_Final/Cl-"])
            os.chdir("/project/ls-gruen/users/r.kanaki/Masterarbeit/Hirosaki_universe/Flask_Simulation")
            os.rename("map-f1z1.fits", "../Flask_Simulation_Test_Final/Kappa_Field/Gauss/map_realization_"+str(i)+"-f1z1.fits")
            os.chdir("/home/r/R.Kanaki/flask")
